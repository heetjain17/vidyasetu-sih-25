...
export const translateText = async (
  ...
  if (memoryCache[cacheKey]) {
    console.debug("[LingoCache] Memory cache hit:", cacheKey.slice(0, 50));
  ...
  if (storedEntry) {
    console.debug("[LingoCache] Storage cache hit:", cacheKey.slice(0, 50));
  ...
  // Call Lingo.dev API
  try {
    console.debug("[LingoAPI] Translating:", text.slice(0, 50) + "...");
...
export const translateBatch = async (
  ...
  console.debug(
    `[LingoCache] Batch: ${texts.length - toTranslate.length} cached, ${toTranslate.length} to translate`
  );
...
export const clearExpiredCache = (): void => {
  ...
  console.debug(`[LingoCache] Cleared ${cleared} expired entries`);
};
...
export const clearTranslationCache = (): void => {
  ...
  console.debug(`[LingoCache] Cleared all ${keys.length} cache entries`);
};
