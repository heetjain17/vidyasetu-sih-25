...
def trigger_whatsapp_n8n(phone: str, exam_name: str):
    ...
    if not n8n_webhook:
        logging.warning(
            f"Mocking WhatsApp send to {phone} for {exam_name} (No N8N_WEBHOOK set)"
        )
        return
    ...
    try:
        ...
        logging.error(f"Failed to trigger WhatsApp n8n: {e}")
        raise e
...
async def sync_whatsapp(payload: WhatsAppSyncRequest):
    ...
    logging.info(
        f"WhatsApp subscription received for {payload.phone_number} -> {payload.exam_name}"
    )
...
async def sync_calendar(payload: CalendarSyncRequest):
    ...
    try:
        ...
        logging.info(
            f"Calendar synced and shared: {payload.email} -> {payload.exam_name}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sync calendar: {str(e)}"
        )
