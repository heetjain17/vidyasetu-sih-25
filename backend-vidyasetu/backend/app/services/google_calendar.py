...
def get_calendar_service():
    ...
    if not service_account_file:
        ...
        if os.path.exists("service_account.json"):
            ...
            raise Exception(
                "GOOGLE_APPLICATION_CREDENTIALS not set and service_account.json not found."
            )
    ...
    for evt in events:
        ...
        if is_all_day:
            ...
            try:
                ...
                logging.error(f"Date parsing error for {start_time}: {e}")
                continue
...
def share_calendar_with_user(service, calendar_id, email):
    ...
    try:
        service.acl().insert(
            calendarId=calendar_id, body=user_rule, sendNotifications=True
        ).execute()
        logging.info(f"Shared calendar with {email}")
    except HttpError as e:
        if "already exists" in str(e).lower():
            logging.info(f"User {email} already has access")
        else:
            logging.warning(f"ACL Insert Warning for user: {e}")

    try:
        service.acl().insert(calendarId=calendar_id, body=public_rule).execute()
        logging.info("Made calendar public")
    except HttpError as e:
        if "already exists" in str(e).lower():
            logging.info("Calendar is already public")
        else:
            logging.warning(f"ACL Insert Warning for public: {e}")
...
def get_calendar_link(calendar_id):
    ...
    return {"view_url": view_url, "add_url": add_url}
