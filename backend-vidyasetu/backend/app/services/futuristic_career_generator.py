...
Uses Google Gemini API to generate emerging/future career suggestions
...
from app.utils.gemini_client import get_gemini_client, GEMINI_MODEL
...
def generate_futuristic_careers(
    ...
    user_profile = (
        "\n".join(profile_parts)
        if profile_parts
        else "General student looking for career guidance"
    )
    ...
    try:
        response = gemini_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a futuristic career advisor. Always respond with valid JSON only.",
                },
        ...
        return {"success": False, "error": str(e), "careers": []}
...
def is_futuristic_career_query(question: str) -> bool:
    ...
    futuristic_keywords = [
        ...
        "career in 2030",
        "career in 2040",
        "next generation career",
        "innovative career",
        "cutting edge career",
        "modern career",
        "21st century career",
        "ai career",
        "tech career of future",
        "space career",
        "metaverse career",
    ...
    if "future" in q_lower and any(
        w in q_lower for w in ["career", "job", "profession", "work"]
    ):
        return True
...
def get_futuristic_careers_for_chatbot(question: str) -> Dict[str, Any]:
    ...
    structured = generate_futuristic_careers(interests=question, num_careers=4)

    return {
        "answer": text_response,
        "futuristic_careers": (
            structured.get("careers", []) if structured.get("success") else []
        ),
